...
const Header = () => {
  ...
  return (
    <div className="w-full bg-whiteText md:sticky md:top-0 z-50">
      <div className="max-w-screen-xl mx-auto h-20 flex justify-between items-center px-4 lg:px-0">
        {/* Logo */}
        <Link to={"/"}>
          <img src={logo} alt="logo" className="w-44" />
        </Link>
        {/* SearchBar */}
        <div className="w-full relative hidden md:inline-flex max-w-3xl">
          <input
            type="text"
            value={searchText}
            onChange={(e) => setSearchText(e.target.value)}
            placeholder="Search Product..."
            className="w-full flex-1 rounded-full text-gray-900 text-lg placeholder:text-base placeholder:tracking-wide shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 placeholder:font-normal focus:ring-1 focus:ring-darkText sm:text-sm px-4 py-2 ml-2"
          />
          {searchText ? (
            <IoClose
              onClick={() => setSearchText("")}
              className="absolute top-2.5 right-4 text-xl hover:text-redText cursor-pointer duration-200"
            />
          ) : (
        ...
        {searchText && (
          <div className="absolte left-0 top-20 w-full mx-auto max-h-[500px] px-10 py-5 bg-white z-20 overflow-y-scroll text-black shadow-lg shadow-skyText scrollbar-hide">
        ...
        {/* Menubar */}
        <div className="flex items-center gap-x-6 text-2xl ml-1">
          <Link to={"/profile"}>
            {currentUser ? (
              <img
      ...
      <div className="w-full bg-darkText text-whiteText">
        <Container className="py-2 max-w-4xl flex items-center gap-5 justify-between">
          <Menu>
            ...
            <Transition
              enter="ease-out transition duration-75"
            ...
            >
              ...
              >
                {catogories.map((item: CategoryProps) => (
